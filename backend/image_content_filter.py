    ...
    def __init__(self):
        ...
        try:
            ...
            if credentials_path and os.path.exists(credentials_path):
                ...
                possible_paths = [
                    os.path.join(os.path.dirname(__file__), "my-project-92814-457204-c90e6bf83130.json"),
                ...
                for path in possible_paths:
                    if os.path.exists(path):
                        self.client = vision.ImageAnnotatorClient.from_service_account_json(path)
                        logger.info(f"Using Google Cloud credentials from: {path}")
                        found_credentials = True
                        break
                
                if not found_credentials:
                    # Try to initialize with default credentials (useful in cloud environments)
                    try:
                        logger.info("Attempting to use default credentials (ADC)")
                        self.client = vision.ImageAnnotatorClient()
                        found_credentials = True
                    except Exception as e:
                        logger.warning(f"Could not use default credentials: {str(e)}")
